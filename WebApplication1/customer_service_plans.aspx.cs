...
namespace WebApplication1
{
    public partial class customer_service_plans : System.Web.UI.Page
    {
        ...
        {
            // Ensure the data is only loaded on the first page load
            if (!IsPostBack)
            {
                DisplayCustomerServicePlans();
            }
        }

        private void DisplayCustomerServicePlans()
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    GridViewCustomerServicePlans.DataSource = dt;
                    GridViewCustomerServicePlans.DataBind(); // Bind the data to the GridView
                }
                else
                {
                    // If no data, show a message (you can customize the message)
