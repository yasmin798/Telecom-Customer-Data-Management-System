...
namespace WebApplication1
{
    public partial class CashbackTransactions : System.Web.UI.Page
    {
        ...
        protected void FetchTransactions(object sender, EventArgs e)
        {
            string connectionString = WebConfigurationManager.ConnectionStrings["MyDatabaseConnection"].ToString();
            SqlConnection conn = new SqlConnection(connectionString);
            int NID = Int16.Parse(NationalID.Text);
            String nationalID = NationalID.Text;

            if (string.IsNullOrEmpty(nationalID))
            {
                lblMessage.Text = "Please enter your national id.";
                return;
            }
            if (!long.TryParse(nationalID, out _))
            {
                lblMessage.Text = "National ID must contain only numeric digits.";
                return;
            }
            else
            {
