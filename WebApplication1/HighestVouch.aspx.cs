...
namespace TestwJazz
{
    public partial class HighestVouch : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
        }

        protected void btnGetHighestVoucher_Click(object sender, EventArgs e)
        {
            string mobileNum = txtMobileNum.Text.Trim();

            if (!string.IsNullOrEmpty(mobileNum))
            {
                LoadHighestVoucher(mobileNum);
            }
            else
            {
        ...
        private void LoadHighestVoucher(string mobileNum)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    // Call the stored procedure 'Account_Highest_Voucher'
                    ...
                    // Execute the procedure and fetch the highest voucher
                    var result = cmd.ExecuteScalar();

                    if (result != null)
                    {
                        lblHighestVoucher.Text = "Highest Voucher ID: " + result.ToString();
                    }
                    else
                    {
                        lblHighestVoucher.Text = "No vouchers found for this account.";
                    }
                }
                catch (Exception ex)
                {
                    lblError.Text = "Error fetching the highest voucher: " + ex.Message;
                }
            }
        }
